...
def object_obs(
    ...
    left_eef_idx = env.scene["robot"].data.body_names.index("L_link_6")
    right_eef_idx = env.scene["robot"].data.body_names.index("R_link_6")
...
def get_hand_state(
    ...
    hand_joint_states = env.scene["robot"].data.joint_pos[:, -24:]  # Hand joints are last 22 entries of joint state
